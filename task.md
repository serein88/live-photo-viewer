...
| -------------------- | ------ | ------------------ | --: | --- | -------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| EPIC-LIVE-ROBUST-001 | P0-001 | 通用 Live 播放鲁棒性（总任务） |  P0 | 进行中 | 子任务 `TASK-LIVE-STATE-002`、`TASK-LIVE-A-006`、`TASK-LIVE-B-007`、`TASK-LIVE-B-008`、`TASK-LIVE-RG-009` 全部通过。 | 相关文件：`src/js/app.js`、`src/css/app.css` |

## 待确认

| 新任务编号           | 原编号 | 任务名                       | 优先级 | 状态  | 验收标准                              | 备注                                                             |
| --------------- | --- | ------------------------- | --: | --- | --------------------------------- | -------------------------------------------------------------- |
| TASK-LIVE-B-007 | -   | B1：接入 `ffmpeg.wasm` 作为重兜底 |  P0 | 待确认 | 仅在 A 链路失败时触发，保证最差情况下仍可播放（允许静音降级）。 | 已改为 `core-st` 单线程核心（去除 SharedArrayBuffer 依赖）并使用绝对URL加载，待你复测确认。 |

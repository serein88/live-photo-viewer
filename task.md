...
|---|---|---|---:|---|---|---|
| EPIC-LIVE-ROBUST-001 | P0-001 | 通用 Live 播放鲁棒性（总任务） | P0 | 进行中 | 子任务 `TASK-LIVE-STATE-002`、`TASK-LIVE-A-005`、`TASK-LIVE-A-006`、`TASK-LIVE-B-007`、`TASK-LIVE-B-008`、`TASK-LIVE-RG-009` 全部通过。 | 相关文件：`src/js/app.js`、`src/css/app.css` |

## 待确认

| 新任务编号               | 原编号       | 任务名                           | 优先级 | 状态  | 验收标准                                        | 备注                           |
| ------------------- | --------- | ----------------------------- | --: | --- | ------------------------------------------- | ---------------------------- |
| TASK-LIVE-STATE-002 | P0-001-02 | 播放状态机收敛（original -> stripped） |  P0 | 待确认 | 同一轮播放只允许一次回退；无双重回退、无黑屏闪烁、无重复 `play reject`。 | 已增加预览遮罩与回退门控，待最终回归。          |
| TASK-LIVE-A-005     | P0-001-05 | A3：引入 `mp4box.js` 做去音轨/重组     |  P0 | 完成  | 替代手写 strip 主链路；输出 MP4 在主流浏览器可重复播放。          | 已接入 mp4box remux 路径，保留旧逻辑兜底。 |

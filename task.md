...
| ------------------- | --- | -------- | --: | --- | --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| TASK-PERF-THUMB-105 | -   | 缩略图持久化缓存 |  P1 | 进行中 | 同目录二次导入与刷新后，缩略图优先命中持久化缓存；首屏缩略图生成耗时下降，且缓存体积受控。 | 已实现 IndexedDB + LRU；并发去重与坏缓存回退已完成；容量调优：scan 6000/64MB、persist 2500/3MB、thumb 12000/320MB；启动日志统一输出 scan+thumb 恢复统计。 |
